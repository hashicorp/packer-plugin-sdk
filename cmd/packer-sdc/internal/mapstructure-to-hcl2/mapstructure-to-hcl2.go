...
type MapstructureToHCL2 struct {
	Ui         cli.Ui
	typeNames  string
	output     string
	trimprefix string
...
func (cmd *MapstructureToHCL2) Help() string {
	return fmt.Sprintf(`
	Usage of %s:
...
func (cmd *MapstructureToHCL2) Flags() *flag.FlagSet {
...
func (cmd *MapstructureToHCL2) Synopsis() string {
	return ""
}

func (cmd *MapstructureToHCL2) Run(args []string) int {
	fs := cmd.Flags()
	err := fs.Parse(args)
	if err != nil {
	...
	if len(pkgs) != 1 {
		cmd.Ui.Error(fmt.Sprintf("error: %d packages found", len(pkgs)))
		return 1
	}
	topPkg := pkgs[0]
	...
	fmt.Fprintf(out, `// Code generated by "%s %s"; DO NOT EDIT.`, mapstructureToHCL2, strings.Join(fs.Args(), " "))
	fmt.Fprintf(out, "\n\npackage %s\n", topPkg.Name)
	...
	if err != nil {
		cmd.Ui.Error(fmt.Sprintf("os.Create: %v", err))
		return 1
	}

	_, err = outputFile.Write(goFmt(outputFile.Name(), out.Bytes()))
	if err != nil {
		cmd.Ui.Error(fmt.Sprintf("failed to write file: %v", err))
		return 1
	}
