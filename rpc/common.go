...
const PackerUsePBEnvVar = "PACKER_RPC_PB"

// useProtobuf is used by Packer/plugins to determine if structs like
// hclspec.Spec should be serialised to protobuf instead of gob.
func useProtobuf() bool {
	useProto := os.Getenv(PackerUsePBEnvVar)
	switch useProto {
	case "", "0", "no", "false":
		return false
	}
